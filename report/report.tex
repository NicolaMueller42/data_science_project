...
\nm{We keep the current motivation for using company descriptions and then only later mention that we can also add other features}

At the same time, recent advances in natural language processing (NLP), e.g. in the form of the popular ChatGPT \cite{chatgpt}, have led to surprisingly good results in numerous applications. Examples range from complex Code generation to essays to cooking recipes. 

This work demonstrates market segments and business clusters can be identified effectively by introducing an automatic clustering pipeline. Utilizing those clusters, our system is then able to provide a strategic location, which we will demonstrate by using the German federal state Saarland as an example. The novelty of our approach is that we rely on language models to encode the companies' information. 
